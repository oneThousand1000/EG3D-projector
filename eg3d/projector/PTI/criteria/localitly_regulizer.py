...
class Space_Regulizer:
    ...
    def ball_holder_loss_lazy(self, new_G, num_of_sampled_latents, w_batch, use_wandb=False):
        ...
        # G.mapping(z=zs, c=c, truncation_psi=psi, truncation_cutoff=truncation_cutoff)
        w_samples = self.original_G.mapping(z=torch.from_numpy(z_samples).to(global_config.device), c=torch.from_numpy(c_samples).to(global_config.device),
                                            truncation_psi=0.5)
        ...
        for w_code in territory_indicator_ws:
            ...
            with torch.no_grad():
                old_img = self.original_G.synthesis(w_code,torch.from_numpy(c_samples).to(global_config.device))
